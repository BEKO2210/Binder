...
export default function DiscoveryScreen({ onOpenMatch, onSessionExpired }: { onOpenMatch?: (match: MatchSummary) => void; onSessionExpired: () => void }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.canvas }}>
      ...
      <BinderScreenHeader title={t('discovery.header.title')} titleVisual={<View><BinderBrand compact />{headerCopyFits ? <BinderText variant="caption" tone="muted" maxFontSizeMultiplier={HEADER_TEXT_CAP} style={{ marginTop: theme.spacing.x2 }}>{t('discovery.header.copy')}</BinderText> : null}{filterValues ? <BinderText variant="caption" tone="accent" maxFontSizeMultiplier={HEADER_TEXT_CAP} style={{ marginTop: theme.spacing.x2, marginBottom: theme.spacing.x1 }}>{t('discovery.filters.summary', { minAge: formatCount(filterValues.minAge, locale), maxAge: formatCount(filterValues.maxAge, locale), distance: formatDistanceKm(filterValues.distance, locale) })}{attributeFilterCount > 0 ? ` · ${tCount('discovery.filters.more', attributeFilterCount, { count: formatCount(attributeFilterCount, locale) })}` : ''}</BinderText> : null}{pendingCount > 0 ? <BinderText variant="caption" tone="muted" maxFontSizeMultiplier={HEADER_TEXT_CAP} style={{ marginTop: theme.spacing.x1 }}>{tCount('discovery.pending', pendingCount, { count: formatCount(pendingCount, locale) })}</BinderText> : null}</View>} trailing={<View style={{ flexDirection: 'row', alignItems: 'center', gap: theme.spacing.x2 }}>
