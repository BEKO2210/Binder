...
begin
  ...
  -- The rate limit is sender-wide, not conversation-wide. Serializing all new
  -- sends by this user prevents parallel requests across different matches from
  -- each observing a stale count and collectively exceeding the limit.
  ...
  -- sender lock. Resolve it idempotently before touching the match row.
  select m.* into existing
  from public.messages m
  where m.sender_id = uid and m.client_message_id = p_client_message_id;
  ...
  -- Serialize the send against unmatch/block transitions for this match.
